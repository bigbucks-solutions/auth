...
- `?country=IN` — the organization's ISO-3166 alpha-2 country code. The server
  maps it through `subscriptions.currencies.byCountry` (`IN` → `inr`, for
  example). When omitted, the server reads the country of
  `X-Organization-Id`; an unmapped country uses the configured default.
- `?currency=usd` — optional ISO-4217 override for a currency switcher. It wins
  over `country` and is ignored if every configured plan cannot sell in it.

Example:

```http
GET /api/v1/billing/plans?country=IN
X-Organization-Id: 01H...
```
...
- **`currency`** is the code to display. It is resolved from `?currency=`, then
  `?country=`, then the stored organization country, and finally the configured
  default. An existing Stripe customer currency lock takes precedence over all
  of them.
...
The `update_plan` deep link requires **Allow customers to update
subscriptions** in Stripe Customer Portal settings, with both price and quantity
updates enabled. The catalog provisioning script configures this on the active
default portal configuration. If it is disabled, Stripe rejects session creation
and this endpoint returns `502`.
